    ...
    size_t & out_key_column_num)
{
    ...
    {
        /// Right arg is a subquery or a table. They are indexed only by the AST hash
        /// (and not by the set data types)
        data_types.clear();
    }

    PreparedSetKey set_key{right_arg->getTreeHash(), data_types};
...
bool KeyCondition::atomFromAST(const ASTPtr & node, const Context & context, Block & block_with_constants, RPNElement & out)
{
    ...
    if (const ASTFunction * func = typeid_cast<const ASTFunction *>(node.get()))
    {
        ...
        if (functionIsInOrGlobalInOperator(func->name)
            && tryPrepareSetIndex(args, prepared_sets, context, out, key_column_num))
        {
            key_arg_pos = 0;
            is_set_const = true;
        }
